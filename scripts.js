...
function loseHeart() {
	for (var i = 4; i >= 0; i--) {
		...
		if (heartClass === "hidden") {
			...
			break; 		 
		 // EDIT THIS TO DISPLAY "You Lost a Heart!" for a few seconds.  Make visible a hidden alert box div (absolute position) below the hearts?
		}
	}
}
...
function giveChallenge(charId) {
	alert("This is your challenge!");
}
*/

function giveChallenge(charId) {
	// show it by removing hidden
	document.getElementById(charId+"-challenge").className = "challenge";
}
function closeChallenge(charId) {
	document.getElementById(charId+"-challenge").className = "challenge hidden";
}
